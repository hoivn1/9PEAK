 ...
 *
 * [9peak-fork] v0.4.2 — when refresh response carries a new id_token (Codex
 * normally does NOT return one on refresh, but some flows may), decode it and
 * surface workspace info via providerSpecificData so it stays fresh. If no
 * id_token is returned, we leave existing providerSpecificData untouched
 * (caller's `existingProviderSpecificData` merge in src/sse/services/tokenRefresh.js
 * preserves the workspace data captured at OAuth time).
 */
export async function refreshCodexToken(refreshToken, log) {
  ...
  // [9peak-fork] v0.4.2 — re-extract workspace info if a new id_token came back.
  if (tokens.id_token) {
    try {
      ...
      if (parts.length === 3) {
        ...
        const top = sorted[0];
        const psd = {};
        if (chatgpt.chatgpt_account_id) psd.chatgptAccountId = chatgpt.chatgpt_account_id;
        if (chatgpt.chatgpt_plan_type) psd.chatgptPlanType = chatgpt.chatgpt_plan_type;
        if (chatgpt.chatgpt_user_id) psd.chatgptUserId = chatgpt.chatgpt_user_id;
        if (orgs.length) {
          psd.organizations = orgs.map((o) => ({
            id: o.id,
      ...
      log?.debug?.("TOKEN_REFRESH", `Codex id_token decode skipped: ${err.message}`);
    }
  }
