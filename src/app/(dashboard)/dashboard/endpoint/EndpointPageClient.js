...
export default function APIPageClient({ machineId }) {
  ...
  const [cavemanLevel, setCavemanLevelState] = useState("lite");
  const [enableChatGPTImageGen, setEnableChatGPTImageGenState] = useState(false);
  const [chatGPTImageGenSaved, setChatGPTImageGenSaved] = useState(false);
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      if (settingsRes.ok) {
        ...
        setCavemanEnabledState(data.cavemanEnabled || false);
        setCavemanLevelState(data.cavemanLevel || "lite");
        setEnableChatGPTImageGenState(data.enableChatGPTImageGen || false);
      }
      if (statusRes.ok) {
  ...
  const handleEnableChatGPTImageGen = async (value) => {
    try {
      const res = await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ enableChatGPTImageGen: value }),
      });
      if (res.ok) {
        setEnableChatGPTImageGenState(value);
        setChatGPTImageGenSaved(true);
        setTimeout(() => setChatGPTImageGenSaved(false), 2000);
      }
    } catch (error) {
      console.log("Error updating enableChatGPTImageGen:", error);
    }
  };
      ...
      <Card id="experimental-features">
        ...
        <div className="flex items-start justify-between pt-2 gap-4">
          <div className="pr-4 flex-1 min-w-0">
            <p className="font-medium">ChatGPT OAuth Image Generation</p>
            <p className="text-sm text-text-muted">
              Dùng Codex OAuth token (Plus/Pro/Business/Go/Team) để sinh ảnh qua model <code>codex/gpt-image-1</code>. <span className="text-amber-600 dark:text-amber-400">⚠️ Experimental, có thể vi phạm ToS ChatGPT. Tắt mặc định.</span>
            </p>
            {chatGPTImageGenSaved && (
          ...
          <div className="shrink-0">
            <Toggle
              checked={enableChatGPTImageGen}
              onChange={() => handleEnableChatGPTImageGen(!enableChatGPTImageGen)}
            />
          </div>
        </div>
        {enableChatGPTImageGen && (
          <div className="mt-3 flex items-start gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-400">
            <span className="material-symbols-outlined text-[16px] shrink-0 mt-0.5">warning</span>
            <p className="text-xs flex-1">
              <span aria-hidden="true">⚠️ </span>
              This feature routes image requests to your ChatGPT OAuth accounts instead of the official OpenAI API. OpenAI ToS may restrict non-official client usage. Accounts may be rate-limited or suspended. Use at your own risk.
            </p>
          </div>
        )}
      </Card>
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex items-center justify-between gap-4">
            <div className="pr-4">
              ...
              <p className="text-sm text-text-muted">
                Inject terse style instruction into system prompt to reduce verbose assistant output.
              </p>
            </div>
            <Toggle
          ...
          {cavemanEnabled && (
            <div className="mt-3 flex flex-wrap gap-2">
              {[
                ...
                const active = cavemanLevel === item.id;
                return (
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => handleCavemanLevel(item.id)}
                    className={`px-3 py-1.5 rounded-md text-xs font-medium border transition-colors ${active
                      ? "bg-primary/15 border-primary/50 text-primary"
                      : "bg-input border-border text-text-muted hover:text-text-main hover:border-primary/40"
                      }`}
                  >
                    {item.label}
                  </button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  Expose your local 9PEAK to the internet. No port forwarding, no static IP needed. Share endpoint URL with your team or use it in Cursor, Cline, and other AI tools from anywhere.
                </p>
              </div>
            </div>
          </div>
