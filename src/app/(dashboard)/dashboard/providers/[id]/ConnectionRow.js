...
// Map Codex ChatGPT plan type → badge label + Tailwind color classes
const CHATGPT_PLAN_BADGES = {
  plus: { label: "Plus", className: "bg-green-500/15 text-green-600 dark:text-green-400" },
  pro: { label: "Pro", className: "bg-purple-500/15 text-purple-600 dark:text-purple-400" },
  business: { label: "Business", className: "bg-yellow-500/15 text-yellow-700 dark:text-yellow-400" },
  go: { label: "Go", className: "bg-sky-500/15 text-sky-600 dark:text-sky-400" },
  team: { label: "Team", className: "bg-amber-500/15 text-amber-700 dark:text-amber-400" },
  enterprise: { label: "Enterprise", className: "bg-gray-800/15 text-gray-800 dark:bg-white/10 dark:text-gray-200" },
  free: { label: "Free", className: "bg-gray-500/15 text-gray-600 dark:text-gray-400" },
};

function ChatGPTPlanBadge({ planType }) {
  if (!planType) return null;
  const key = String(planType).toLowerCase();
  const cfg = CHATGPT_PLAN_BADGES[key];
  if (!cfg) return null;
  return (
...
ChatGPTPlanBadge.propTypes = {
  planType: PropTypes.string,
};

export default function ConnectionRow({ connection, provider, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  const resolvedProvider = provider || connection?.provider;
  const chatgptPlanType = resolvedProvider === "codex"
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <ChatGPTPlanBadge planType={chatgptPlanType} />
          </div>
