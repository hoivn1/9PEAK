...
import BulkImportCodexModal from "./components/BulkImportCodexModal";
import BulkLoginCodexModal from "./components/BulkLoginCodexModal";
// [9peak-fork] v0.4.2 — Codex Workspace Grouping
import { groupConnectionsByWorkspace } from "@/lib/codexWorkspace";
...
export default function ProviderDetailPage() {
  ...
  // [9peak-fork] v0.4.2 — Codex workspace grouping toggle, default ON for codex
  const [groupByWorkspace, setGroupByWorkspace] = useState(params.id === "codex");
  // [9peak-fork] v0.4.5 — local "Farm 1" / "Farm 2" labels per workspace.
  // Loaded from /api/settings.codexWorkspaceLabels and persisted via PATCH.
  const [codexWorkspaceLabels, setCodexWorkspaceLabels] = useState({});
  ...
  const fetchConnections = useCallback(async () => {
    try {
      ...
      // [9peak-fork] v0.4.5 — load local workspace labels (Farm 1 / Farm 2).
      // Migration safe: missing field → default {} (no crash).
      const labels = settingsData.codexWorkspaceLabels;
      setCodexWorkspaceLabels(
  ...
  // [9peak-fork] v0.4.2 — proxy update helper used by both flat list and grouped view.
  const onConnectionProxyUpdate = async (connId, proxyPoolId) => {
    try {
  ...
  // [9peak-fork] v0.4.5 — save user-defined workspace label.
  // Empty/whitespace label → delete the entry. Optimistic UI update.
  const handleRenameWorkspace = useCallback(async (workspaceId, label) => {
    if (!workspaceId) return;
    const next = { ...codexWorkspaceLabels };
    ...
    setCodexWorkspaceLabels(next);
    try {
      await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ codexWorkspaceLabels: next }),
      });
    } catch (error) {
      console.log("Error saving workspace label:", error);
    }
  }, [codexWorkspaceLabels]);

  const flatConnectionsList = (
          ...
          <div key={conn.id} className="flex items-stretch">
            <div className="flex-1 min-w-0">
              <ConnectionRow
                ...
                onUpdateProxy={(proxyPoolId) => onConnectionProxyUpdate(conn.id, proxyPoolId)}
                onEdit={() => {
  ...
  // [9peak-fork] v0.4.4 — Migration helper for legacy Codex connections.
  // Re-extracts workspace info from saved id_tokens so admins don't need
  // to re-OAuth every account post-v0.4.2 upgrade.
  const [migratingWorkspaces, setMigratingWorkspaces] = useState(false);
  const handleMigrateWorkspaces = async () => {
    if (migratingWorkspaces) return;
    setMigratingWorkspaces(true);
    try {
      const res = await fetch("/api/codex/migrate-workspaces", { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data?.ok) {
        const { migrated = 0, skipped = 0, failed = 0, total = 0 } = data;
        alert(
          `Re-fetch workspace info xong:\n` +
          `  • Migrated: ${migrated}\n` +
          `  • Skipped (đã có hoặc không có id_token): ${skipped}\n` +
          `  • Failed: ${failed}\n` +
          `  • Total scanned: ${total}\n\n` +
          (migrated > 0 ? "Refresh trang để thấy workspace mới." : "Không có acc nào cần migrate.")
        );
        await fetchConnections();
      } else {
        alert(`Re-fetch failed: ${data?.error || res.statusText}`);
      }
    } catch (e) {
      alert(`Re-fetch error: ${e?.message || e}`);
    } finally {
      setMigratingWorkspaces(false);
    }
  };

  const renderCodexWorkspaceView = () => {
    const { workspaces, individuals, unassigned } = groupConnectionsByWorkspace(connections);
    const sections = [];

    // [9peak-fork] v0.4.5 — workspaces are already pre-sorted in
    // groupConnectionsByWorkspace() with severityRank: owner-down → critical
    // → warning → healthy. We render in that order so the most urgent farm
    // surfaces on top of the providers/[id] page.
    for (const ws of workspaces) {
      sections.push(
        <WorkspaceCard
          ...
          group={ws}
          proxyPools={proxyPools}
          customLabel={codexWorkspaceLabels[ws.id]}
          onRename={handleRenameWorkspace}
          onToggleActive={handleUpdateConnectionStatus}
          onUpdateProxy={onConnectionProxyUpdate}
          onEdit={(conn) => {
            setSelectedConnection(conn);
            setShowEditModal(true);
          }}
          ...
          onMoveUp={(connId) => {
            ...
            if (idx > 0) handleSwapPriority(idx, idx - 1);
          }}
          onMoveDown={(connId) => {
            const idx = connections.findIndex((c) => c.id === connId);
            if (idx >= 0 && idx < connections.length - 1) handleSwapPriority(idx, idx + 1);
          }}
        />
      );
    }
    ...
    if (unassigned.length > 0) {
      const unassignedGroup = {
        ...
        healthyCount: unassigned.filter((c) => c.isActive !== false).length,
        cascade: { triggered: false, downCount: 0, totalCount: unassigned.length, ratio: 0, severity: null },
      };
      sections.push(
        <WorkspaceCard
          key="__unassigned__"
          ...
          headerExtra={
            <button
              ...
              disabled={migratingWorkspaces}
              className="text-xs px-2.5 py-1 rounded-md border border-primary/30 text-primary hover:bg-primary/5 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              title="Re-extract workspace info from saved id_tokens"
            >
              {migratingWorkspaces ? "Re-fetching..." : "Re-fetch workspace info"}
            </button>
          }
        />
      );
  ...
  const showCodexGroupedView = isCodexProvider && groupByWorkspace;
  const connectionsList = showCodexGroupedView
    ? renderCodexWorkspaceView()
    : flatConnectionsList;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {isFreeNoAuth ? (
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              {/* [9peak-fork] v0.4.2 — Codex Workspace grouping toggle (default ON) */}
              {isCodexProvider && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-text-muted font-medium">Group by workspace</span>
                  <Toggle
                    checked={groupByWorkspace}
                    onChange={setGroupByWorkspace}
                  />
                </div>
              )}
              {/* Thinking config */}
                ...
                <div className="flex gap-2 justify-center">
                  ...
                  {isCodexProvider && (
                    <Button
                      size="sm"
                      icon="login"
                      onClick={() => setShowBulkLoginModal(true)}
                    >
                      Bulk Login
                    </Button>
                  )}
      ...
      <BulkLoginCodexModal
        isOpen={isCodexProvider && showBulkLoginModal}
        providerInfo={providerInfo}
        proxyPools={proxyPools}
        onSuccess={fetchConnections}
        onClose={() => setShowBulkLoginModal(false)}
      />

      <BulkImportCodexModal
        isOpen={isCodexProvider && showBulkImportModal}
