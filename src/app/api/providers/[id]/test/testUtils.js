...
export async function testSingleConnection(id) {
  ...
  if (!connection) return { valid: false, error: "Connection not found", latencyMs: 0, statusCode: null, testedAt };

  const persistHealthCheck = async (healthCheck, extraData = {}) => {
    await updateProviderConnection(id, {
      ...extraData,
      healthCheck,
      lastTested: healthCheck.testedAt,
    });
    return healthCheck;
  };
  ...
  if (effectiveProxy.connectionProxyEnabled && effectiveProxy.connectionProxyUrl && !effectiveProxy.vercelRelayUrl) {
    ...
    if (!proxyResult.ok) {
      ...
      const healthCheck = {
        ...
        latencyMs: 0,
        error: proxyError,
        statusCode: proxyResult.status || null,
        testedAt: failedAt,
      };
      await persistHealthCheck(healthCheck, {
        testStatus: "error",
  ...
  const updateData = {
    ...
    lastErrorAt: result.valid ? null : finishedAt,
  };
  ...
  await persistHealthCheck(healthCheck, updateData);
