...
const isCloud = typeof EdgeRuntime !== "undefined" || process.env.NEXT_RUNTIME === "edge";
...
const DEFAULT_SETTINGS = {
  ...
  cavemanLevel: "lite",
  // [9peak-fork] v0.4.0 — Smart Routing v2 (Auto Mode)
  // routingMode: "auto" | "preset" | "custom"
  //   - "auto":   group connections by ChatGPT plan tier and route smartly
  //               (default for fresh installs — community-friendly, zero-config).
  //   - "preset": preset profiles (spread/speed/quota/fillfirst), v0.4.1+.
  //   - "custom": legacy 4-strategy logic via fallbackStrategy.
  // Migration: existing DBs (with settings but no routingMode key) are kept on
  // "custom" by ensureDbShape below, preserving v0.3.x behavior. Only fresh
  // installs (DB created from scratch) get "auto".
  routingMode: "auto",
  routingPreset: null,
  perAccountOverrides: {},
  welcomeWizardCompletedAt: null,
  // [9peak-fork] v0.4.5 — Codex workspace local labels.
  // Map of workspace ID (or `__individual__` / `__unassigned__`) → user-defined
  // label like "Farm 1" / "Farm 2". Allows admins to disambiguate Business
  // farms when JWT-derived titles are generic or duplicated. Empty string
  // (or missing key) → fall back to JWT title.
  // Migration safe: existing settings without this key get `{}` via
  // ensureDbShape's per-key default loop below.
...
function ensureDbShape(data) {
  ...
  for (const [key, defaultValue] of Object.entries(defaults)) {
    ...
    if (key === "settings" && typeof next.settings === "object" && !Array.isArray(next.settings)) {
      // [9peak-fork] v0.4.0 — detect upgrade vs. fresh-install for routingMode default.
      // A fresh-install has a brand-new settings object (no user-set keys yet).
      // An upgrade has an existing settings object missing only newly-added keys.
      // We treat any settings object that already has at least one persisted user-
      // visible key (e.g. requireLogin, fallbackStrategy, stickyRoundRobinLimit)
      // as an upgrade and force routingMode="custom" to preserve v0.3.x behavior.
      const isUpgradeFromExisting = next.settings && Object.keys(next.settings).some(k =>
        k === "fallbackStrategy" ||
        k === "stickyRoundRobinLimit" ||
        k === "comboStrategy" ||
        k === "requireLogin" ||
        k === "providerStrategies"
      );
      ...
      for (const [settingKey, settingDefault] of Object.entries(defaultValue)) {
        if (next.settings[settingKey] === undefined) {
          ...
          } else if (settingKey === "routingMode" && isUpgradeFromExisting) {
            // [9peak-fork] v0.4.0 — preserve v0.3.x behavior on upgrade.
            // Only fresh installs (no existing settings markers) get "auto".
            next.settings[settingKey] = "custom";
          } else {
