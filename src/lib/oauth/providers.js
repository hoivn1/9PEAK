...
// [9peak-fork] v0.4.2 — extract workspace `organizations` array + `primaryWorkspace`
//   pick (non-personal first, owner-role first) so dashboard can group connections
//   by ChatGPT workspace. Pure additive — existing chatgptAccountId/chatgptPlanType
//   fields preserved for v0.4.0/v0.4.1 callers.
export function extractCodexAccountInfo(idToken) {
  const payload = decodeJwtPayload(idToken);
  if (!payload) return {};
  const chatgpt = payload["https://api.openai.com/auth"] || {};
  const orgs = Array.isArray(chatgpt.organizations) ? chatgpt.organizations : [];
  ...
  const normalized = orgs.map((o) => ({
    id: o.id,
    title: o.title || o.name || "Unnamed Workspace",
    role: o.role,
    personal: !!o.personal,
    isDefault: !!o.is_default,
  }));
  const top = sorted[0];
  const primaryWorkspace = top
    ? {
        id: top.id,
        title: top.title || top.name || "Unnamed Workspace",
        role: top.role,
  ...
  return {
    ...
    chatgptUserId: chatgpt.chatgpt_user_id,
...
const PROVIDERS = {
  ...
  codex: {
    ...
    mapTokens: (tokens) => {
      ...
      // [9peak-fork] v0.4.2 — persist workspace info alongside chatgpt account info
      if (info.chatgptAccountId || info.chatgptPlanType || info.primaryWorkspace || (info.organizations && info.organizations.length)) {
        mapped.providerSpecificData = {
          ...
          primaryWorkspace: info.primaryWorkspace || null,
        };
      }
      return mapped;
    },
...
export async function backfillCodexEmails() {
  ...
  try {
    ...
    const targets = connections.filter((c) => {
      ...
      // [9peak-fork] v0.4.2 — also re-extract if workspace info missing
      const hasWorkspaceInfo = !!c.providerSpecificData?.primaryWorkspace
        || (Array.isArray(c.providerSpecificData?.organizations) && c.providerSpecificData.organizations.length > 0);
      return !hasEmail || !hasAccountInfo || !hasWorkspaceInfo;
    });
    for (const conn of targets) {
      const info = extractCodexAccountInfo(conn.idToken);
      if (!info.email && !info.chatgptAccountId && !info.primaryWorkspace) continue;
      const patch = {};
      ...
      if (info.chatgptAccountId || info.chatgptPlanType || info.primaryWorkspace || (info.organizations && info.organizations.length)) {
        patch.providerSpecificData = {
          ...
          chatgptAccountId: info.chatgptAccountId,
          chatgptPlanType: info.chatgptPlanType,
          // [9peak-fork] v0.4.2 — workspace data
