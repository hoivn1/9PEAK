...
// [9peak-fork] Copy bundled server.js into DATA_DIR so MITM doesn't lock
// node_modules during global package update. Only safe when server.js is
// truly bundled (single file with no sibling requires).
//
// Detection: if `logger.js` exists alongside server.js, this is raw source
// (server.js does `require('./logger')` and friends). Copying just server.js
// breaks the require chain — MITM throws "Cannot find module './logger'".
// In that case, run from source location.
function ensureRuntimeServer(bundledPath) {
  try {
    if (!bundledPath || !fs.existsSync(bundledPath)) return bundledPath;

    const dir = path.dirname(bundledPath);
    const hasSiblings =
      fs.existsSync(path.join(dir, "logger.js")) ||
      fs.existsSync(path.join(dir, "paths.js")) ||
      fs.existsSync(path.join(dir, "dns"));

    // Raw source — sibling files needed at the same dir. Don't copy alone.
    if (hasSiblings) return bundledPath;

    // Legacy heuristic kept as a sanity check: only bundled-into-node_modules
    // packages are expected to be self-contained.
